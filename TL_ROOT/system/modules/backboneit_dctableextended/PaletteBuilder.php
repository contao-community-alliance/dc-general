...
class PaletteBuilder {
	...
	public function generateAjaxPalette($strSelector, $strInputName, $strFieldTemplate) {
		return is_array($this->arrAjaxPalettes[$strSelector])
			? sprintf('<div id="sub_%s">%s</div>',
				$strInputName,
				$this->generatePalette($this->arrAjaxPalettes[$strSelector], $strFieldTemplate)
			)
			: '';
	}
...
//					$strClass .= ' cbx';
	...
	protected function selectRootPalette() {
		...
		foreach($arrSelectors as $strSelector) {
			...
			$arrKeys[] = $arrDef['inputType'] == 'checkbox' && !$arrDef['eval']['multiple']
				? $strSelector
				: $varValue;
		}
